...
async function main(): Promise<void> {
  ...
  console.log(`Starting eth-nft-copy-bot on ${config.chain.name}…`);
  await loadState();
  ...
  const trackNet = await withBootTimeout("track-rpc", 10_000, async () => {
    try {
      ...
      if (hasTrackBackup()) {
        console.warn(
          `[boot] track primary failed — forcing track backup: ${
  ...
  // Alchemy failover alerts (ETH). Silent only when publicRpcOnly.
  ...
  // eth.blockscout.com transfers + Alchemy getLogs / pending.
