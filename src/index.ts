...
async function main(): Promise<void> {
  ...
  console.log(`Starting arc-nft-copy-bot on ${config.chain.name}…`);
  await loadState();
  ...
  console.log("[boot] Telegram slash commands registered (incl. /consolidate /disburse)");
  ...
  // Robinhood Blockscout watcher is RH-only — skip on Arc.
  const stopBlockscout =
    config.chain.key === "arc" || config.chain.key === "arc-testnet"
      ? (() => {
          console.log("[boot] Arc: skipping Robinhood Blockscout watcher");
          return () => undefined;
        })()
