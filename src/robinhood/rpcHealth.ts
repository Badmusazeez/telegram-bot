...
export function formatRpcLimitAlert(
  ...
  return [
    ...
    ``,
    role === "track"
      ? `Update <code>TRACK_RPC_URL</code> in VPS <code>.env</code>, then: <code>pm2 restart eth-nft-bot --update-env</code>`
      : `Update <code>MINT_RPC_URL</code> in VPS <code>.env</code>, then: <code>pm2 restart eth-nft-bot --update-env</code>`,
    ``,
