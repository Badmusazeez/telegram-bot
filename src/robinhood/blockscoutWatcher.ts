...
function explorerApiBase(): string {
  // Etherscan explorer links ≠ Blockscout API. ETH uses eth.blockscout.com.
  if (config.chain.key === "ethereum" || config.chain.key === "eth") {
  ...
  try {
    const res = await fetch(url, {
      headers: {
        ...
        origin: explorerApiBase().replace(/\/api\/v2$/, ""),
        referer: `${explorerApiBase().replace(/\/api\/v2$/, "")}/`,
      },
