...
/**
 * Per-wallet nonce lock — prevents duplicate/colliding sends when the same
 * mint wallet fires multiple strategies or retries quickly.
 *
 * Cross-wallet parallel blasts (20 keys) remain fully parallel.
 */
...
export async function withWalletNonce<T>(params: {
  ...
  try {
    ...
    if (nonce == null) {
      nonce = await params.provider.getTransactionCount(params.address, "pending");
      localNonce.set(key, nonce);
    }
    const used = nonce;
    try {
      const result = await params.fn(used);
      // Assume broadcast consumed this nonce.
      localNonce.set(key, used + 1);
      return result;
    } catch (err) {
      // Likely nonce race / rejection — force refresh next call.
