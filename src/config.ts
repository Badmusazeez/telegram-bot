...
/** Always prefer this bot's .env over any parent-shell RH vars. */
dotenv.config({
...
const schema = z.object({
  ...
  /** Legacy alias — ignored on ETH; use TRACK_RPC_URL. */
...
/** Strip RH / Arc / Ink leftovers — never cross-wire other bots' RPCs. */
function sanitizeRpc(url: string): string {
  const u = url.trim();
  if (!u) return "";
  if (/robinhood|arc-scan|arc\.io|inkonchain|ink\.drpc/i.test(u)) {
    console.warn(
      `[config] ignoring foreign-chain RPC (ETH bot): ${u.slice(0, 60)}…`
    );
    return "";
  }
  return u;
...
const trackRpcUrl =
  sanitizeRpc(env.TRACK_RPC_URL) ||
  sanitizeRpc(env.ROBINHOOD_RPC_URL) ||
  chain.defaultRpcUrl;
const mintRpcUrl = sanitizeRpc(env.MINT_RPC_URL) || trackRpcUrl;
const mintBackupCandidate = sanitizeRpc(env.MINT_RPC_BACKUP_URL);
...
/** Optional backup — empty when track+mint share the same Alchemy URL. */
...
/** ETH Alchemy bot — keep CU/failover Telegram alerts. */
const publicRpcOnly = false;
...
export const config = {
  ...
  chain,
  /** ETH Alchemy bot — CU/failover alerts enabled. */
  publicRpcOnly,
