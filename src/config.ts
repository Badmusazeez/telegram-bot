...
const schema = z.object({
  ...
  /** Legacy alias — ignored on Arc; use TRACK_RPC_URL. */
...
function sanitizeRpc(url: string): string {
  const u = url.trim();
  if (!u) return "";
  if (/robinhood|alchemy\.com\/v2\/test|chainstack\.com\/testmint/i.test(u)) {
    return "";
  }
  return u;
}
...
/** No Robinhood Chainstack defaults on Arc — never cross-wire to RH RPCs. */
