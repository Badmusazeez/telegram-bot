  ...
  bot.command("golive", async (ctx) => {
    ...
    await ctx.reply(
      [
        ...
        `Fund each with ETH gas, then keep the bot running.`,
      ].join("\n"),
      { parse_mode: "HTML" }
    );
  });
  ...
  bot.command("dryrun", async (ctx) => {
    ...
    await ctx.reply(
      arg === "on"
        ...
        : "Dry-run OFF — live free-mint replay enabled (needs PRIVATE_KEY + ETH gas)."
    );
  });
