...
async function replyBalances(ctx: Context): Promise<void> {
  ...
  const footer = [
    anyOk
      ...
      : "",
    usdPrice != null
      ? `\nUSDC/USD: <code>${escape(formatUsd(usdPrice))}</code>`
...
export function createTelegramBot(): Bot {
  ...
  bot.command("golive", async (ctx) => {
    ...
    await ctx.reply(
      [
        ...
        `Fund each with Arc USDC gas, then keep the bot running.`,
      ].join("\n"),
      { parse_mode: "HTML" }
    );
  });
  ...
  bot.command("dryrun", async (ctx) => {
    ...
    await ctx.reply(
      arg === "on"
        ...
        : "Dry-run OFF — live free-mint replay enabled (needs PRIVATE_KEY + Arc USDC gas)."
    );
  });
  ...
  bot.command("maxbuy", async (ctx) => {
    ...
    await ctx.reply(`Max buy set to ${value} (Arc native USDC)`);
  });
      ...
      if (parsed.error === "help") {
        ...
        await ctx.reply(
          [
            ...
            ...keyLines,
            "",
            "Requires /dryrun off + USDC gas on each key.",
          ].join("\n"),
          { parse_mode: "HTML" }
        );
        return;
