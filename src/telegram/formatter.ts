...
export function formatStatus(stats: ScannerStats, paused: boolean): string {
  ...
  const lines = [
    ...
    `Universe: top ${config.maxPairs || "all"} pairs · scan every ${(config.scanIntervalMs / 1000).toFixed(0)}s · alerts/scan ≤ ${config.maxAlertsPerScan}`,
    `Hard gates: SMC ${config.requireSmcHard ? "on" : `soft ≥${Math.round(config.smcMinScore * 100)}%`} · PA ${config.requirePaHard ? "on" : "soft"} · Vol ${config.requireVolumeHard ? "on" : "soft"} · Mom ${config.requireMomentumHard ? "on" : "soft"}`,
